...
- [x] AAAHA.local-bin restructured: devline branch, installed worktree, stow-deployed launchers (org#373 → AAAHA.local-bin PR#3+#4+#6, AVA012Q 0.12.17, 2026-09-08)
- [x] OTTOPOET health check improved: launchers now probe 192.168.0.103:1234 directly, not just otto-proxy process (AAAHA.local-bin PR#6, AVA012Q 0.12.17, 2026-09-08)
- [x] aurora-vr task worker restarted on OTTOPOET endpoint — was targeting dead llama.cpp localhost:8080 since Aug20; confirmed via `systemctl --user restart aurora-worker` + journalctl (local repo only, AVA012Q 0.12.17, 2026-09-08)
